...
class ResNet20(tf.keras.Model):
    def __init__(self,binW,binA):
        ...
        # BinaryActivation(False) should also work.
        self.actv01=layers.BinaryActivationHT(False) if binA else tf.keras.layers.Activation("relu")
        self.actv03=layers.BinaryActivationHT(False) if binA else tf.keras.layers.Activation("relu")
        self.actv05=layers.BinaryActivationHT(False) if binA else tf.keras.layers.Activation("relu")
        self.actv07=layers.BinaryActivationHT(False) if binA else tf.keras.layers.Activation("relu")
        self.actv09=layers.BinaryActivationHT(False) if binA else tf.keras.layers.Activation("relu")
        self.actv11=layers.BinaryActivationHT(False) if binA else tf.keras.layers.Activation("relu")
        self.actv13=layers.BinaryActivationHT(False) if binA else tf.keras.layers.Activation("relu")
        self.actv15=layers.BinaryActivationHT(False) if binA else tf.keras.layers.Activation("relu")
        self.actv17=layers.BinaryActivationHT(False) if binA else tf.keras.layers.Activation("relu")
        self.actv02=layers.BinaryActivationHT(False) if binA else tf.keras.layers.Activation("relu")
        self.actv04=layers.BinaryActivationHT(False) if binA else tf.keras.layers.Activation("relu")
        self.actv06=layers.BinaryActivationHT(False) if binA else tf.keras.layers.Activation("relu")
        self.actv08=layers.BinaryActivationHT(False) if binA else tf.keras.layers.Activation("relu")
        self.actv10=layers.BinaryActivationHT(False) if binA else tf.keras.layers.Activation("relu")
        self.actv12=layers.BinaryActivationHT(False) if binA else tf.keras.layers.Activation("relu")
        self.actv14=layers.BinaryActivationHT(False) if binA else tf.keras.layers.Activation("relu")
        self.actv16=layers.BinaryActivationHT(False) if binA else tf.keras.layers.Activation("relu")
        self.actv18=layers.BinaryActivationHT(False) if binA else tf.keras.layers.Activation("relu")
        ...
        self.batnoh02=tf.keras.layers.BatchNormalization()
        self.batnoh03=tf.keras.layers.BatchNormalization()
        self.batnoh04=tf.keras.layers.BatchNormalization()
        self.batnoh05=tf.keras.layers.BatchNormalization()
        self.batnoh06=tf.keras.layers.BatchNormalization()
        self.batnoh07=tf.keras.layers.BatchNormalization()
        self.batnoh08=tf.keras.layers.BatchNormalization()
        self.batnoh09=tf.keras.layers.BatchNormalization()
        self.batnoh10=tf.keras.layers.BatchNormalization()
        self.batnoh11=tf.keras.layers.BatchNormalization()
        self.batnoh12=tf.keras.layers.BatchNormalization()
        self.batnoh13=tf.keras.layers.BatchNormalization()
        self.batnoh14=tf.keras.layers.BatchNormalization()
        self.batnoh15=tf.keras.layers.BatchNormalization()
        self.batnoh16=tf.keras.layers.BatchNormalization()
        self.batnoh17=tf.keras.layers.BatchNormalization()
        self.batnoh18=tf.keras.layers.BatchNormalization()
    ...
    def call(self, inputs, training=False):
        ...
        x=x    +         self.batnor02(self.conv02(self.drop01(self.actv01(self.batnoh01(x,training=True)),training=training)),training=True)
        x=x    +         self.batnor03(self.conv03(self.drop02(self.actv02(self.batnoh02(x,training=True)),training=training)),training=True)
        x=x    +         self.batnor04(self.conv04(self.drop03(self.actv03(self.batnoh03(x,training=True)),training=training)),training=True)
        x=x    +         self.batnor05(self.conv05(self.drop04(self.actv04(self.batnoh04(x,training=True)),training=training)),training=True)
        x=x    +         self.batnor06(self.conv06(self.drop05(self.actv05(self.batnoh05(x,training=True)),training=training)),training=True)
        x=x    +         self.batnor07(self.conv07(self.drop06(self.actv06(self.batnoh06(x,training=True)),training=training)),training=True)
        x=self.convd1(x)+self.batnor08(self.conv08(self.drop07(self.actv07(self.batnoh07(x,training=True)),training=training)),training=True)
        x=x    +         self.batnor09(self.conv09(self.drop08(self.actv08(self.batnoh08(x,training=True)),training=training)),training=True)
        x=x    +         self.batnor10(self.conv10(self.drop09(self.actv09(self.batnoh09(x,training=True)),training=training)),training=True)
        x=x    +         self.batnor11(self.conv11(self.drop10(self.actv10(self.batnoh10(x,training=True)),training=training)),training=True)
        x=x    +         self.batnor12(self.conv12(self.drop11(self.actv11(self.batnoh11(x,training=True)),training=training)),training=True)
        x=x    +         self.batnor13(self.conv13(self.drop12(self.actv12(self.batnoh12(x,training=True)),training=training)),training=True)
        x=self.convd2(x)+self.batnor14(self.conv14(self.drop13(self.actv13(self.batnoh13(x,training=True)),training=training)),training=True)
        x=x    +         self.batnor15(self.conv15(self.drop14(self.actv14(self.batnoh14(x,training=True)),training=training)),training=True)
        x=x    +         self.batnor16(self.conv16(self.drop15(self.actv15(self.batnoh15(x,training=True)),training=training)),training=True)
        x=x    +         self.batnor17(self.conv17(self.drop16(self.actv16(self.batnoh16(x,training=True)),training=training)),training=True)
        x=x    +         self.batnor18(self.conv18(self.drop17(self.actv17(self.batnoh17(x,training=True)),training=training)),training=True)
        x=x    +         self.batnor19(self.conv19(self.drop18(self.actv18(self.batnoh18(x,training=True)),training=training)),training=True)
...
class ResNet20_EVAL(ResNet20):
    def call(self, inputs, training=False):
        x=               self.batnor01(self.conv01(inputs),training=True)
        x=x    +         self.batnor02(self.conv02(self.actv01(self.batnoh01(x,training=training))),training=training)
        x=x    +         self.batnor03(self.conv03(self.actv02(self.batnoh02(x,training=training))),training=training)
        x=x    +         self.batnor04(self.conv04(self.actv03(self.batnoh03(x,training=training))),training=training)
        x=x    +         self.batnor05(self.conv05(self.actv04(self.batnoh04(x,training=training))),training=training)
        x=x    +         self.batnor06(self.conv06(self.actv05(self.batnoh05(x,training=training))),training=training)
        x=x    +         self.batnor07(self.conv07(self.actv06(self.batnoh06(x,training=training))),training=training)
        x=self.convd1(x)+self.batnor08(self.conv08(self.actv07(self.batnoh07(x,training=training))),training=training)
        x=x    +         self.batnor09(self.conv09(self.actv08(self.batnoh08(x,training=training))),training=training)
        x=x    +         self.batnor10(self.conv10(self.actv09(self.batnoh09(x,training=training))),training=training)
        x=x    +         self.batnor11(self.conv11(self.actv10(self.batnoh10(x,training=training))),training=training)
        x=x    +         self.batnor12(self.conv12(self.actv11(self.batnoh11(x,training=training))),training=training)
        x=x    +         self.batnor13(self.conv13(self.actv12(self.batnoh12(x,training=training))),training=training)
        x=self.convd2(x)+self.batnor14(self.conv14(self.actv13(self.batnoh13(x,training=training))),training=training)
        x=x    +         self.batnor15(self.conv15(self.actv14(self.batnoh14(x,training=training))),training=training)
        x=x    +         self.batnor16(self.conv16(self.actv15(self.batnoh15(x,training=training))),training=training)
        x=x    +         self.batnor17(self.conv17(self.actv16(self.batnoh16(x,training=training))),training=training)
        x=x    +         self.batnor18(self.conv18(self.actv17(self.batnoh17(x,training=training))),training=training)
        x=x    +         self.batnor19(self.conv19(self.actv18(self.batnoh18(x,training=training))),training=training)
        x=self.actv19(x)
        return self.dense(self.globalavgpool(x))
...
if(ba and bw):
    with np.load("DResNet20tr2bhtd_1a_Best.npz") as wts:
...
trr.sw_epc=0
trr.train(model,cifar10.datagen,cifar10.x_train,cifar10.y_train,cifar10.x_val,cifar10.y_val,80,init_lr,"DResNet20tr2bhtd_"+str(rn),True,cifar10.x_test,cifar10.y_test)
trr.train(model,cifar10.datagen,cifar10.x_train,cifar10.y_train,cifar10.x_val,cifar10.y_val,600,init_lr/10,"DResNet20tr2bhtd_"+str(rn),True,cifar10.x_test,cifar10.y_test)
...
with np.load("DResNet20tr2bhtd_"+str(rn) + mode + "_Best.npz") as wts:
